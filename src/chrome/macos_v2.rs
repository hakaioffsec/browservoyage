...
#[derive(Debug)]
pub struct ModernMacOSChromeExtractor {
    config: ChromeBrowserConfig,
    user_data_path: PathBuf,
}

impl ModernMacOSChromeExtractor {
    ...
    /// Get all profiles for this browser
...
impl BrowserExtractor for ModernMacOSChromeExtractor {
    fn name(&self) -> &str {
        &self.config.name
    }
...
impl ChromeExtractorBase for ModernMacOSChromeExtractor {
    ...
    fn extract_cookies(
        ...
        let cookie_iter = stmt.query_map([], |row| {
            Ok((
                ...
                row.get::<_, Option<String>>(2)?,             // value
                row.get::<_, Vec<u8>>(3).unwrap_or_default(), // encrypted_value
        ...
        for cookie_result in cookie_iter {
            if let Ok((host_key, name, value, encrypted_value, path, expires_utc, is_secure)) =
                cookie_result
            {
                let final_value = if value.is_none() && !encrypted_value.is_empty() {
...
mod tests {
    ...
    fn test_config_creation() {
        let chrome = ModernMacOSChromeExtractor::chrome();
        assert_eq!(chrome.config.name, "Chrome");

        let brave = ModernMacOSChromeExtractor::brave();
    ...
    fn test_path_generation() {
        ...
        let path = ModernMacOSChromeExtractor::get_user_data_path_for_config(&config);
