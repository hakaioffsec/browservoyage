...
#[derive(Debug)]
pub struct OptimizedWindowsChromeExtractor {
...
impl OptimizedWindowsChromeExtractor {
    ...
    fn initialize_master_key_with_impersonation(&mut self) -> BrowserVoyageResult<()> {
        ...
        let master_key = self.derive_master_key_with_active_impersonation()?;
    ...
    /// Derive master key with active impersonation (no nested impersonation)
    fn derive_master_key_with_active_impersonation(&self) -> BrowserVoyageResult<Vec<u8>> {
        ...
        // No need for nested impersonation - we're already impersonating
        debug!("Performing SYSTEM DPAPI decryption (impersonation already active)");
        let system_decrypted = self.dpapi_unprotect(encrypted_key)?;

        debug!("Performing USER DPAPI decryption");
        ...
        info!("Successfully derived master key for {}", self.config.name);
        Ok(master_key)
    }
    ...
    /// Decrypt Chrome data on Windows (uses cached master key)
...
impl BrowserExtractor for OptimizedWindowsChromeExtractor {
    fn name(&self) -> &str {
        &self.config.name
    }
    ...
    fn extract(&mut self) -> BrowserVoyageResult<ExtractedData> {
        ...
        info!(
            "Starting extraction from {} at {:?}",
            self.config.name, self.user_data_path
        );

        // 🚀 KEY OPTIMIZATION: Initialize master key ONCE with impersonation
        self.initialize_master_key_with_impersonation()?;
        info!(
            "Master key initialized for {} - proceeding with extraction",
            self.config.name
        );

        // Now extract normally (no more impersonation needed)
        self.extract_with_common_logic()
    }
}

impl ChromeExtractorBase for OptimizedWindowsChromeExtractor {
...
mod tests {
    ...
    fn test_config_creation() {
        let chrome = OptimizedWindowsChromeExtractor::chrome();
        assert_eq!(chrome.config.name, "Chrome");

        let brave = OptimizedWindowsChromeExtractor::brave();
    ...
    fn test_path_generation() {
        ...
        let path = OptimizedWindowsChromeExtractor::get_user_data_path_for_config(&config);
