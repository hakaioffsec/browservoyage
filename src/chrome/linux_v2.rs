...
#[derive(Debug)]
pub struct ModernLinuxChromeExtractor {
...
impl ModernLinuxChromeExtractor {
    ...
    fn get_user_data_path_for_config(config: &ChromeBrowserConfig) -> Option<PathBuf> {
        ...
        let config_dir = home_dir.join(".config").join(&config.data_dir_name);
...
impl BrowserExtractor for ModernLinuxChromeExtractor {
    fn name(&self) -> &str {
        &self.config.name
    }
...
impl ChromeExtractorBase for ModernLinuxChromeExtractor {
...
mod tests {
    ...
    fn test_config_creation() {
        let chrome = ModernLinuxChromeExtractor::chrome();
        assert_eq!(chrome.config.name, "Chrome");

        let brave = ModernLinuxChromeExtractor::brave();
        assert_eq!(brave.config.name, "Brave");
    }

    #[test]
    fn test_encryption_mode_detection() {
        let extractor = ModernLinuxChromeExtractor::chrome();
